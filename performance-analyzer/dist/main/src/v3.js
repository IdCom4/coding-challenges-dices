...
function jumpToNextValidHigherConfiguration(dices, numberOfFace, diffToJump) {
    let offsetIndex = 1;
    let wantedIndex = dices.length - offsetIndex;
    while (wantedIndex >= 0) {
        if (!wantedIndex && dices[wantedIndex] + diffToJump > numberOfFace)
            return false;
...
function jumpToNextLowerEnoughConfiguration(dices, numberOfFace, wantedTotal) {
    let offsetIndex = 1;
    let wantedIndex = dices.length - offsetIndex;
    ...
    while (currentConfigurationOutput > wantedTotal) {
        ...
        for (let i = 1; i <= wantedIndex; i++) {
            ...
            if (dices[wantedIndex - i] <= numberOfFace)
                break;
            else if (!(wantedIndex - i))
                return false;
            else {
...
function getTotalPossibleConfigurations(total, numberOfDices, numberOfFace = 6) {
    ...
    while (currentConfigurationOutput < numberOfDices * numberOfFace) {
        ...
        if (currentConfigurationOutput === total) {
            ...
            if (currentConfigurationOutput < total) {
                if (!jumpToNextValidHigherConfiguration(dices, numberOfFace, total - currentConfigurationOutput))
                    break;
            }
            else {
                if (!jumpToNextLowerEnoughConfiguration(dices, numberOfFace, total))
                    break;
            }
        }
    }
